...
# Converts COCO format [x_min, y_min, w, h] (normalized) to Pascal VOC [x_min, y_min, x_max, y_max] (absolute)
def convert_bbox_coco_to_pascal(boxes: torch.Tensor, image_size: tuple[int, int]) -> torch.Tensor:
    """
    Convert bounding boxes from COCO format (x_min, y_min, width, height) in range [0, 1]
    to Pascal VOC format (x_min, y_min, x_max, y_max) in absolute coordinates.

    Args:
        boxes (torch.Tensor): Bounding boxes in COCO format
...
def class_aware_sample(dataset: datasets.Dataset) -> WeightedRandomSampler:
    # Class-Aware Sampling
    labels = [sample['objects']['category'][0] for sample in dataset]
...
def augment_and_transform_batch(
    ...
    for image_id, image_path, objects in zip(examples["image_id"], examples["image"], examples["objects"]):
        ...
        formatted_annotations = format_image_annotations_as_coco(
            image_id, output["category"], [b[2]*b[3] for b in output["bboxes"]], output["bboxes"]
        )
...
def main():
    ...
    tensorboard = SummaryWriter(log_dir=os.path.join("runs", args.output_dir))
    ...
    # If we don't have a test split, split off a percentage of train as test.
    train_test_split = None if "test" in dataset.keys() else 0.1
    if isinstance(train_test_split, float) and train_test_split > 0.0:
        split = dataset["train"].train_test_split(train_test_split, seed=args.seed)
    ...
    id2label = {int(k): v for k, v in class_map.items()}  # Ensure keys are integers
    label2id = {v: int(k) for k, v in class_map.items()}
    ...
    train_augment_and_transform = A.Compose(
        [
            ...
            A.RandomResizedCrop(size=(max_size, max_size), scale=(0.6, 1.0), ratio=(0.75, 1.33), p=0.5),
        ...
        bbox_params=A.BboxParams(format="coco", label_fields=["category"], clip=True, min_area=5),
    )
    ...
    # Class-Aware Sampling
    logger.info("Creating class-aware samplers for train, validation and test datasets.")
    sampler_train = class_aware_sample(dataset["train"])
    sampler_val = class_aware_sample(dataset["validation"])
    sampler_test = class_aware_sample(dataset["test"])
    ...
    train_dataloader = DataLoader(
        train_dataset, shuffle=False, batch_size=args.per_device_train_batch_size, sampler=sampler_train, **dataloader_common_args
    )
    valid_dataloader = DataLoader(
        valid_dataset, shuffle=False, batch_size=args.per_device_eval_batch_size, sampler=sampler_val, **dataloader_common_args
    )
    test_dataloader = DataLoader(
        test_dataset, shuffle=False, batch_size=args.per_device_eval_batch_size, sampler=sampler_test, **dataloader_common_args
    )
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        tensorboard.add_scalar("train/loss", total_loss.item() / len(train_dataloader), epoch)
        for k, v in metrics.items():
            tensorboard.add_scalar("train/"+k, v, epoch)
    ...
    for k, v in metrics.items():
        tensorboard.add_scalar("test/" + k, v)
